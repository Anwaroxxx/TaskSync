<nav x-data="{ open: false }" class="bg-zinc-900 border-b border-zinc-800">
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    <x-nav-link :href="route('dashboard')" :active="request()->routeIs('dashboard')" class="text-zinc-400 hover:text-white transition-none">
                        {{ __('Dashboard') }}
                    </x-nav-link>

                    @if (Auth::user()->isOrganizer())
                        <x-nav-link :href="route('organizer')" :active="request()->routeIs('organizer')" class="text-zinc-400 hover:text-white transition-none">
                            {{ __('Manage Tasks') }}
                        </x-nav-link>
                    @endif

                    @if (Auth::user()->isParticipant())
                        <x-nav-link :href="route('participant')" :active="request()->routeIs('participant')" class="text-zinc-400 hover:text-white transition-none">
                            {{ __('My Tasks') }}
                        </x-nav-link>
                    @endif
                </div>
            ...
            <div class="-me-2 flex items-center sm:hidden">
                <button @click="open = ! open" class="inline-flex items-center justify-center p-2 rounded-md text-zinc-400 hover:text-white hover:bg-zinc-800 focus:outline-none transition-none">
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden bg-zinc-900 border-t border-zinc-800">
        <div class="pt-2 pb-3 space-y-1">
            ...
            @if (Auth::user()->isOrganizer())
                <x-responsive-nav-link :href="route('organizer')" :active="request()->routeIs('organizer')" class="text-slate-400">
                    {{ __('Manage Tasks') }}
                </x-responsive-nav-link>
            @endif

            @if (Auth::user()->isParticipant())
                <x-responsive-nav-link :href="route('participant')" :active="request()->routeIs('participant')" class="text-slate-400">
                    {{ __('My Tasks') }}
                </x-responsive-nav-link>
            @endif
        </div>
